...
register_uninstall_hook( __FILE__, 'osmap_uninstall' );
...
function osmap_uninstall() {
    delete_option( 'osmap_options' );
    delete_option( 'osmap_sunset_time' );
    delete_site_option( 'osmap_options' );
    delete_site_option( 'osmap_sunset_time' );
}
...
function osmap_admin_notice() {
    ...
    if ( current_user_can('administrator') ) {
        $now = time();
        $last = get_option( 'osmap_sunset_time', 0 );
        $admin_screen = get_current_screen();
        if ( ( $now > $last + 86400 ) || ( $admin_screen->base == 'settings_page_os-openspace-maps' ) ) {
            echo '<div class="notice notice-warning is-dismissible"><p><strong>OS OpenSpace Maps is going away!</strong> Ordnance Survey will be withdrawing the <a href="https://www.ordnancesurvey.co.uk/business-government/products/openspace">OpenSpace service</a> in August 2021. The new replacement service, <a href="https://osdatahub.os.uk/">OS Data Hub</a>, is already available and much superior.</p><p>The plugin for the Data Hub service, now available from the <a href="https://wordpress.org/plugins/os-datahub-maps/">WordPress plugin page</a>, uses the same shortcode and is compatible with the OpenSpace plugin, so you should not have to change your shortcodes. It contains several new features such as fullscreen view and elevation profiles.</p><p>We recommend upgrading to the new service and plugin as soon as possible.</p></div>';
            update_option( 'osmap_sunset_time', $now );
        }
    }
}
