...
== Important Notice ==
Ordnance Survey has now [withdrawn the OpenSpace service](https://www.ordnancesurvey.co.uk/business-government/products/openspace) as of August 2021, having replaced it with their new OS Maps API on the [OS Data Hub](https://osdatahub.os.uk/). The new service is much superior and there is a new [OS DataHub Maps plugin](https://wordpress.org/plugins/os-datahub-maps/) available to use it. The new plugin uses the same shortcode and is compatible with the OpenSpace plugin, so you should not have to change your shortcodes. It contains several new features such as fullscreen view and elevation profiles.
...
Full instructions for using the [OS DataHub Maps plugin](https://wordpress.org/plugins/os-datahub-maps/) and getting an account are available on the [plugin homepage](https://skirridsystems.co.uk/wordpress-plugins/os-datahub-maps/)
